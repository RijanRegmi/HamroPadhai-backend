...
describe("UserService Unit Tests", () => {
  ...
  const baseRegisterData = {
    ...
    username: `serviceuser${ts}`,
    email: `service${ts}@example.com`,
  ...
  afterEach(async () => {
    await UserModel.deleteMany({
      $or: [
        { username: /serviceuser|user2|svcuser|newuser/ },
        { email: /service.*test|service.*@test|user2.*test|svcuser|new@test/ },
        { phone: /9800111222|9822222222|9811111111/ },
      ],
    });
    jest.clearAllMocks();
  }, 30000);
    ...
    test("should throw 409 if username already exists", async () => {
      ...
      await expect(
        userService.register({
          ...baseRegisterData,
          email: "new@test.com",
          phone: `91${String(ts).padStart(8,"0").slice(-8)}` as any,
        })
    ...
    test("should throw 409 if email already exists", async () => {
      ...
      await expect(
        userService.register({
          ...baseRegisterData,
          username: "newuser",
          phone: `92${String(ts).padStart(8,"0").slice(-8)}` as any,
        })
  ...
  describe("updateProfile", () => {
    ...
    test("should throw 409 if new email is taken by another user", async () => {
      ...
      const user2 = await userService.register({
        fullName: "User Two",
        username: `svcuser2${ts2}`,
        email: `svcuser2${ts2}@test.com`,
    ...
    test("should throw 409 if new phone is taken by another user", async () => {
      const user1 = await userService.register(baseRegisterData);
      // Register user2 with a completely fixed unique phone
      const uniquePhone = `9300${String(Date.now()).slice(-6)}`;
      const user2 = await userService.register({
        fullName: "User Three",
        username: `svcuser3x${Date.now()}`,
        email: `svcuser3x${Date.now()}@test.com`,
        phone: uniquePhone,
        password: "Password123!",
        gender: "male" as const,
      });

      // Now try to update user1 with user2's phone — should 409
      await expect(
        userService.updateProfile(user1._id.toString(), {
          phone: user2.phone,
        })
