...
describe("UserRepository Unit Tests", () => {
  ...
  describe("isPhoneTakenByOther", () => {
    test("should return true if phone taken by another user", async () => {
      ...
      // Use a completely fixed unique phone — clean it up in afterEach too
      const fixedPhone = "9600000042";
      await UserModel.deleteMany({ phone: fixedPhone });

      const user2 = await userRepository.createUser({
        fullName: "User Two",
        username: `phonetest${Date.now()}`,
